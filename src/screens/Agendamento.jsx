      ...
      <div className="relative flex flex-col min-h-full" style={{ background: bg }}>
        ...
        <div className="flex flex-col items-center justify-center flex-1 px-6 py-10 gap-6">
          ...
          <div className="text-center">
            <p className="font-bold mb-2" style={{ fontSize: fs(24), color: verde }}>Consulta Agendada!</p>
            <p style={{ fontSize: fs(18), color: subtexto }}>Sua consulta foi marcada com sucesso.</p>
          </div>
          <div className="w-full rounded-3xl p-5" style={{ background: card }}>
            <p className="font-bold mb-3" style={{ fontSize: fs(18), color: texto }}>Resumo do agendamento:</p>
            <div className="flex flex-col gap-2">
              <div className="flex justify-between">
                <span style={{ color: subtexto, fontSize: fs(16) }}>Especialidade:</span>
                <span className="font-semibold" style={{ color: texto, fontSize: fs(16) }}>{especialidade}</span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: subtexto, fontSize: fs(16) }}>Data:</span>
                <span className="font-semibold" style={{ color: texto, fontSize: fs(16) }}>{data}</span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: subtexto, fontSize: fs(16) }}>Horário:</span>
                <span className="font-semibold" style={{ color: texto, fontSize: fs(16) }}>{horario}</span>
              </div>
            </div>
          </div>
          <div className="w-full rounded-2xl px-4 py-3" style={{ background: altoContraste ? '#1a1a00' : '#fef3c7' }}>
            <p style={{ color: altoContraste ? '#fde68a' : '#92400e', fontSize: fs(14) }}>
      ...
      <div className="flex flex-col gap-4 px-5 pb-6">
        {passo === 1 && (
          <div className="rounded-3xl p-5" style={{ background: card }}>
            <p className="font-bold mb-1" style={{ fontSize: fs(20), color: texto }}>Passo 1 de 3</p>
            <p className="mb-4" style={{ fontSize: fs(18), color: subtexto }}>Escolha o tipo de médico:</p>
            <div className="flex flex-col gap-3">
              {especialidades.map(e => (
                <button
                  key={e}
                  onClick={() => setEspecialidade(e)}
                  className="rounded-2xl px-5 py-4 font-semibold text-left transition-all"
                  style={{ ...botaoSelecionado(especialidade === e), minHeight: 64, fontSize: fs(18) }}
                >
            ...
            <button
              ...
              onClick={() => setPasso(2)}
              className="w-full py-5 rounded-2xl font-bold mt-5 transition-all"
              style={{
        ...
        {passo === 2 && (
          <div className="rounded-3xl p-5" style={{ background: card }}>
            <p className="font-bold mb-1" style={{ fontSize: fs(20), color: texto }}>Passo 2 de 3</p>
            <p className="mb-4" style={{ fontSize: fs(18), color: subtexto }}>Escolha o dia da consulta:</p>
            <div className="flex flex-col gap-3">
              {datas.map(d => (
                <button
                  key={d.valor}
                  ...
                  className="rounded-2xl px-5 py-4 font-semibold text-left transition-all"
                  style={{ ...botaoSelecionado(data === d.valor), minHeight: 64, fontSize: fs(18) }}
                >
        ...
        {passo === 3 && (
          <div className="rounded-3xl p-5" style={{ background: card }}>
            <p className="font-bold mb-1" style={{ fontSize: fs(20), color: texto }}>Passo 3 de 3</p>
            <p className="mb-4" style={{ fontSize: fs(18), color: subtexto }}>Escolha o horário:</p>
            <div className="grid grid-cols-2 gap-3">
              {horarios.map(h => (
                <button
                  key={h}
                  onClick={() => setHorario(h)}
                  className="rounded-2xl px-4 py-4 font-bold text-center transition-all"
                  style={{ ...botaoSelecionado(horario === h), minHeight: 64, fontSize: fs(18) }}
                >
                  {h}
                </button>
              ))}
